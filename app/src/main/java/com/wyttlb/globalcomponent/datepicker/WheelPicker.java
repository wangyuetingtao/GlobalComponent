package com.wyttlb.globalcomponent.datepicker;
...
import com.wyttlb.globalcomponent.utils.LinearGradient;
...
//            canvas.drawRect(mSelectedItemRect, mPaint);
            //画左右两条线
            float[] linePoints = new float[]{mSelectedItemRect.left, mSelectedItemRect.top, mSelectedItemRect.left, mSelectedItemRect.bottom
            ,mSelectedItemRect.right, mSelectedItemRect.top, mSelectedItemRect.right, mSelectedItemRect.bottom};
            canvas.drawLines(linePoints, 0, linePoints.length, mPaint);
//            canvas.drawRect(mDrawnRect, mPaint);
        ...
        for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1;
            ...
            String drawText = "";
            try{
                drawText = mDataFormat == null ? data.toString() : mDataFormat.format(data);

            }catch (IllegalArgumentException e){
                drawText = data.toString();
            }
            //在中间位置的Item作为被选中的。
     ...
     * 设置当前选中的列表项，将滚动到所选位置
     */
    public void setCurrentItem(T item) { setCurrentItem(item, true);}

    /**
     * 设置当前选中的列表项
     * @param item 期望选中的item
     * @param smoothScroll 是否平滑移动
     */
    public void setCurrentItem(T item, boolean smoothScroll) {
        int index = mDataList.indexOf(item);
        setCurrentPosition(index, smoothScroll);
    }
